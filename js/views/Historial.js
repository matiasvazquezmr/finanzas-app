...
export function renderHistorial() {
    ...
    container.innerHTML = `
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-lg font-bold text-slate-100 tracking-tight">Últimos 15 Movimientos</h2>
            <button id="btn-update-history" class="glass-pill text-violet-300 text-xs font-bold px-3 py-2 rounded-lg flex items-center gap-1.5 active:scale-95 transition-all">
...
export async function loadHistorialData(forceRefresh = false) {
    ...
    try {
        ...
        if(allMoves.length === 0) {
            listEl.innerHTML = `<div class="glass-card text-center text-slate-400 py-10 rounded-2xl text-sm">No hay registros cargados aún.</div>`;
            return;
        }
        ...
        listEl.innerHTML = allMoves.map(m => {
            ...
            let bgIconColor = isIngreso ? 'bg-teal-400/10 text-teal-300' : isGasto ? 'bg-orange-400/10 text-orange-300' : 'bg-cyan-400/10 text-cyan-300';
            ...
            <div class="glass-card p-4 rounded-2xl flex justify-between items-center transition-all">
                <div class="flex items-center gap-3.5">
                    ...
                    <div>
                        <p class="font-bold text-sm text-slate-100">${title}</p>
                        <p class="text-[11px] text-slate-400 font-medium">${subtitle} • ${m._fecha.toLocaleDateString()}</p>
                    </div>
                </div>
                <div class="text-right">
                    <span class="font-extrabold text-base tracking-tight ${isIngreso ? 'text-teal-400' : 'text-slate-100'}">
                        ${finalMonto}
                    </span>
                    ${m['Moneda'] === 'USD' ? '<p class="text-[9px] font-bold text-violet-400 uppercase tracking-wider">USD</p>':''}
                </div>
            </div>`;
        }).join('');
    } catch (err) {
